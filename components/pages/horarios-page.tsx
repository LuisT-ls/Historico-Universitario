  ...
  if (!authLoading && !user) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
        <TableProperties className="h-12 w-12 text-muted-foreground/40" aria-hidden="true" />
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 id="horarios-heading" className="text-2xl font-black tracking-tight text-foreground dark:text-slate-100">
            Grade de Horários
          </h1>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-20" role="status" aria-label="Carregando grade de horários">
        ...
        <div className="flex flex-col xl:flex-row gap-6">
          ...
          <aside className="xl:w-72 shrink-0 space-y-3">
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      <Input
                        ...
                        aria-describedby={hasError ? `horario-error-${id}` : undefined}
                        aria-invalid={hasError || undefined}
                      />
                      {hasError && (
                        <p id={`horario-error-${id}`} role="alert" className="text-[10px] text-destructive">
            ...
            <div className="mt-4 p-3 rounded-xl bg-muted/40 border border-border space-y-1.5">
              ...
              <div className="flex items-start gap-1 pt-1">
                <Info className="h-3 w-3 text-muted-foreground shrink-0 mt-px" aria-hidden="true" />
          ...
          <div className="flex-1 overflow-x-auto">
            <div className="min-w-[500px] rounded-xl border border-border bg-card shadow-sm overflow-hidden">
              <table className="w-full text-xs border-collapse" aria-labelledby="horarios-heading">
                <caption className="sr-only">Grade de horários — semestre {semestre}</caption>
                <thead>
                  <tr>
                    <th scope="col" className="w-24 px-3 py-2.5 text-left text-[10px] font-bold uppercase tracking-widest text-muted-foreground bg-muted/50 border-b border-border">
                      Horário
                    </th>
                    {visibleDays.map(day => (
                      <th
                  ...
                  {TIME_SLOTS.map((slot, rowIdx) => {
                    ...
                    return (
                      <React.Fragment key={rowIdx}>
                        {isFirstOfPeriod && (
                          <tr className="bg-muted/30">
                            <th
                              ...
                              className="px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground border-b border-border text-left font-bold"
                            >
                        ...
                        <tr className="group">
                          {/* Time label */}
                          <th scope="row" className="px-3 py-0 h-9 text-[10px] font-mono text-muted-foreground whitespace-nowrap border-b border-border align-middle font-normal">
                            <span>{slot.label} – {slot.end}</span>
                          </th>
